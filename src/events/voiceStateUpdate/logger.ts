...
export default event(Events.VoiceStateUpdate, async ({ client }, oldState, newState) => {
  const { LOGS } = GuildSettings.get(newState.guild)
  ...
  if (logChannel != null && logChannel instanceof TextChannel) {
    ...
    if (oldChannel == null && newChannel != null) {
      embed
        ...
        .setDescription(`${Text.mention.user(member.id)} vient de rejoindre le salon ${Text.channel(newChannel.id)}.`)
    }
    // scenario 2 : user leaves voice channel
      ...
      embed
        ...
        .setDescription(`${Text.mention.user(member.id)} vient de quitter le salon ${Text.channel(oldChannel.id)}.`)
    }
    // scenario 3 : user moves from one voice channel to another
      ...
      embed
        ...
        .setDescription(
          `${Text.mention.user(member.id)} vient de se déplacer du salon ${Text.channel(
            oldChannel.id
